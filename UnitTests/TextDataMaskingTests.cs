...
@"
<?xml version=""1.0""?>
<machine id=""140283674"">
	<job id=""P0001C"">
		<x>-10.0183</x>
		<y>100.1726</y>
		<depth>17.1836</depth>
		<!--
        Code must be set to D1547 for all settings JSON or XML
        -->
		<settings><![CDATA[{""id"":31473,""code"":""D1547"",""speed"":4,""increment"":1.5}]]></settings>
	</job>
	<job id=""P1210L"">
		<x>180.0811</x>
		<y>12.9825</y>
		<depth>13.4836</depth>
		<!--Code must be set to D1547 for all settings JSON or XML-->
		<settings><![CDATA[<drill id=""31473""><code>D1547</code><speed>3</speed><increment>2.5</increment></drill>]]></settings>
	</job>
	<job id=""P2901Y"">
		<x>128.9381</x>
		<y>-103.0483</y>
		<depth>11.1038</depth>
		<!--Code must be set to D1547 for all settings JSON or XML-->
		<settings><![CDATA[{""id"":31473,""code"":""D1547"",""speed"":5,""increment"":1.0}]]></settings>
	</job>
</machine>
", TextType.Xml));
  ...
  ""geometry"": {
    ...
    ""viewport"": {
      ...
      ""northwest"": {
        ...
        [Test]
        public void TestMaskText_IgnoreAlphaNumeric()
        {
            DataMaskerOptions options = new DataMaskerOptions();
            options.IgnoreAngleBracketedTags = false;
            options.IgnoreJsonAttributes = false;
        ...
        public void ValidatePlainText(string original, string replacement, DataMaskerOptions options)
        {
            ...
            string pattern = @"\s+|\w+|[^A-Za-z0-9\s]+";
            ...
            while (match.Success)
            {
                ...
                string replacementSubstring = replacement.Substring(match.Index, match.Value.Length);
        ...
        public void ValidateHtmlElement(HtmlNode original, HtmlNode replacement, DataMaskerOptions options)
        {
            ...
            for (int a = 0; a < original.Attributes.Count; a++)
            {
                ...
                var rAttr = replacement.Attributes[a];
            ...
            if (original.NodeType == HtmlNodeType.Comment
                || original.NodeType == HtmlNodeType.Text)
            {
        ...
        public void ValidateXml(string original, string replacement, DataMaskerOptions options)
        {
            ...
            oDoc.LoadXml(original.Trim());

            XmlDocument rDoc = new XmlDocument();
            rDoc.LoadXml(replacement.Trim());
        ...
        public void ValidateXmlElement(XmlNode original, XmlNode replacement, DataMaskerOptions options)
        {
            ...
            {
                ValidatePlainText(original.Value, replacement.Value, options);
            }
        }
        ...
        public void ValidateJsonElement(JsonElement original, JsonElement replacement, DataMaskerOptions options)
        {
            ...
            else if (original.ValueKind == JsonValueKind.Null)
            {
                Assert.IsNull(original.GetString());
                Assert.IsNull(replacement.GetString());
            }
            else
            {
                ...
                ValidatePlainText(oString, rString, options);
            }
        }
        ...
        protected void CompareStrings(string string1, string string2, DataMaskerOptions options)
        {
            ...
            else if ((BothMatch(string1, string2, @"^\d+$"))
            ...
            else if (BothMatch(string1, string2, @"\w+")
                    && BothMatch(string1, string2, @"\d+")
                    && !BothMatch(string1, string2, @"\W+"))
            {
                Assert.AreEqual(options.IgnoreAlphaNumeric, string1 == string2);
            }
            else
            {
